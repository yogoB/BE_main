...
└── common/            예외 · 응답 래퍼 · 퍼널 카운터(+화면 단계 수집) · 발신지 판별
```

`chat/`(챗봇)과 `alert/`(알림)은 **없다** — D-44 로 챗봇을 만들지 않기로 했고 알림은 P2 미구현이다.
`SAVINGS_ALERT` 동의는 `privacy/` 가 받아 두지만 **발송 기능은 아직 없다**(G-67).

**외부를 부르는 자리는 셋뿐이다.** 요청 경로가 아니라 배치·운영자 경로에서만 돈다(D-05).

| 클라이언트 | 무엇을 묻나 | 쿨다운 · 읽기 제한 |
|---|---|---|
| `catalog.SubscriptionPriceOracle` | 구독 공식 표기가(AI 계약 §8) | 서비스별 6시간 · 30초 |
| `catalog.PlanPromotionOracle` | 기간 한정 특가(AI 계약 §9) | 호출 전체 6시간 · **90초** — 저쪽이 상품마다 1.2초를 쉬어 30개면 36초다 |
| `recommend.NarratorClient` | 결과 설명 문장(D-50) | 사용자 경로. 실패는 삼키고 화면은 금액만 보인다 |
...
**이 절이나 사본을 고쳤으면 `python3 scripts/contract_audit.py` 를 돌린다** — 항목이 **통째로 빠진 것**을 잡는다.
**값이 바뀐 것은 못 잡는다**(크기 상한·대상 서비스·쿨다운 주체를 바꿔도 통과한다, 2026-09-20 실측). 값 대조는 사람이 한다.
