...
랜딩의 "이용자들이 진단에서 확인한 절감액". **금액만** 나간다 — 계정·요금제·시각·**표본 수**를 싣지 않는다.

```json
{ "data": { "samples": [12000, 23400, 51010], "monthlyAverage": 28803, "monthlyMedian": 23400,
            "basis": "CURRENT_PLAN", "updatedAt": "2026-09-21T02:00:00Z" }, "warnings": [] }
```

| 규칙 | 값 |
|---|---|
| 표본 단위 | **로그인한 채 결과 화면을 본 회원, 계정당 최신 1건**(D-59) — 여러 번 봐도 한 번. 저장 버튼을 누른 사람만 세던 때는(D-53) 표본이 거의 안 쌓여 랜딩이 늘 비어 있었다 |
| 기준(`basis`) | `CURRENT_PLAN` — 저장 당시 **지금 쓰는 요금제 대비** 절감액. 정가 대비가 아니다(알뜰폰은 대부분 0) |
| 제외 | `currentPlanId` 없이 저장한 건(모름 ≠ 0), 절감액 0 이하 |
| 노출 임계 | 표본 **2건 미만**이면 `samples: []` 이고 **`monthlyAverage`·`monthlyMedian` 도 null**. 한두 명의 금액을 평균이라는 이름으로 내보내지 않는다 |
| 표본 수 | **응답에 없다**(2026-09-21). 화면에 안 적는 것만으로는 부족했다 — 공개 경로라 인증 없이 받으면 "몇 명이고 각각 얼마"가 그대로 읽혔다. 화면의 노출 판단은 `samples` 가 비었는지로 한다.<br>**남는 한계**: `samples` 자체가 개인별 금액이고 길이를 세면 표본 수가 나온다. 줄어든 것은 노출 표면이지 노출 자체가 아니다 |
| 1인당 | `monthlyAverage`(랜딩의 "1인당 평균 절감액")·`monthlyMedian`. 표본과 같은 기준·같은 표본에서 낸다(D-57) |
| 금액 단위 | **1,000원 단위로 뭉개서** 내보낸다(2026-09-21). 정확한 값은 그 사람의 결과를 그대로 공개하는 셈이다. **평균·중앙값도 뭉갠 값에서 낸다** — 원본으로 평균을 내면 거기서 역산될 여지가 남는다. 화면은 "약"이라고 적는다 |
| 개수·캐시 | 최근 **12건**, 60초 캐시 |
...
### 6-3b. 절감 추천 알림 동의 변경 — `POST /api/v1/me/consent/savings-alert` (인증+CSRF)

`{agree: true|false}` → `{agreed: …}`. **6-3 과 글자 그대로 같은 모양**이고 내부도 같은 함수를 쓴다 —
같은 성격의 선택 항목을 다른 모양으로 두면 화면이 둘을 다르게 다루게 되고 언젠가 한쪽만 고쳐진다.
철회는 행을 지우지 않고 `withdrawn_at` 을 찍는다(증적). **다시 켤 수 있다.** 비인증 POST 는
401 이 아니라 **403** 이다 — CSRF 필터가 인증보다 먼저 걸린다(G-67).

**발송 기능은 아직 없다.** 그래도 끄는 길을 먼저 열었다 — 보내기 시작한 다음에 만들면 그 사이에
받은 사람은 끌 수 없었다.
...
"kpi":     { "source": "최종보고서 §9.2",   // 못 내는 KPI 는 **키를 지우지 않고 null + Note**. 키가 없으면 화면이 "아직 안 만들었나"로 읽고, 0 이면 거짓이 된다
             ...
             "calcErrorRate": 0.0, "calcErrorRateBasis": "GOLDEN_GATE", "calcErrorRateCases": 68,
             // 런타임 값이 아니다 — 정답셋과 하나라도 어긋나면 배포가 막히므로 운영 빌드에서는 언제나 0.
             // 낼 수 있는 건 "얼마나 틀렸나"가 아니라 "무엇이 몇 개를 지키나"다. 개수는 빌드가 docs/testing.md 에서 세어 심는다
             "calcErrorRateNote": "배포 전 골든 감사 기준이다 — …",
