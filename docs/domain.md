...
| `BUNDLE_INCLUDED` | 요금제에 포함(등급 미상이 대부분) | — | **계산에는 쓰지 않는다**(정가 유지). 탐지에서는 전액 낭비로 본다 |

**`BUNDLE_INCLUDED` 의 실제 의미(2026-09-17 정정).** 이 표는 원래 "번들가를 구성 서비스에 분담"
이라고 적었지만, 변환기(`catalog_benefits_from_matrix.py`)는 **등급 표기가 없을 때** 이 값을 붙인다(§2 표).
즉 실데이터의 뜻은 "요금제에 이 서비스가 포함되는데 **어느 등급인지 모른다**" 이고, 운영 혜택 73행 중
**51행(70%)** 이 이것이다. 분담 계산은 구현된 적이 없다.

금액을 확정할 수 없으므로 **계산(`PlanBenefit.apply`)에서는 정가를 그대로 둔다** — 지어내지 않는다.
**탐지에서는 다르다**: 요금제가 포함한다고 말한 서비스를 따로 결제 중이면 그 돈은 버려진다.
등급 미상이면 상한(내는 금액 전액)을 `ESTIMATED` 로 낸다(표시 전용, D-17 · 골든 케이스 G-09 e).
