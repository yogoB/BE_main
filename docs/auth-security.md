# 인증 보안 시나리오 검증 · 2026-09-10 (세션 2026-09-11 · 메일 제거 2026-09-16 · **Google 전용 2026-09-17**)
...
| 로그인 입구 반복 호출·X-Forwarded-For 위조 | IP 15분 40회 초과 시 429. 위조 헤더가 아니라 실제 발신지로 센다 (G-20f) |
| X-Forwarded-For 변경으로 IP 제한 우회 | 같은 소켓 IP로 계산, 429 |
| 요청 파라미터로 다른 회원 사칭(`?userId=`) | 무시된다. 신원은 인증 쿠키에서만 온다 |
...
2. 이메일 선점·복구 불가: **D-34 로 해소됐다.** 소유 확인도 계정 복구도 Google 이 한다.
...
- **이메일 소유 확인 문제는 D-34 로 사라졌다.** 이메일은 Google 이 확인해 준 값만 들어온다.
  대신 **단일 장애점**이 생겼다 — Google 계정이 없거나 Google 이 막히면 아무도 로그인할 수 없다.
- **회원 비밀번호를 아예 보관하지 않는다.** 유출될 비밀번호도, 추측당할 비밀번호도 없다.
  운영자 백오피스 계정(D-32)은 예외이며 비밀번호를 쓴다 — 그쪽 위험은 별도로 본다.
