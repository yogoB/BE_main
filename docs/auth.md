...
**가입·로그인은 Google OAuth 하나뿐이다(D-34).** 사용자가 보는 것은 버튼 하나이고,
Google `sub` 가 이미 있으면 로그인, 없으면 가입이다 — 서버가 나눈다.
**우리는 회원 비밀번호를 보관하지 않는다.** 그래서 재설정도 복구 코드도 없다(잊을 것이 없다).

추천·계산기·카탈로그·챗봇 **엔드포인트는 비회원에게 열려 있다.** 다만 **결과 리포트 화면은 로그인해야
보인다**(D-36) — 이는 화면 게이트이며 API 권한이 아니다. CBT 이탈율을 보고 되돌릴 수 있게 그렇게 뒀다.
...
**남는 위험:** Google 계정이 없거나 Google 이 막히면 **서비스를 아예 쓸 수 없다.** 단일 장애점을
받아들인 결정이다. 비회원 경로(추천·계산기·카탈로그)는 열려 있어 로그인 없이도 계산까지는 간다 —
다만 결과 리포트 화면은 로그인 뒤에 있다(D-36).
...
5. 로그아웃·Google 콜백은 기존 HTTP 세션을 없애므로 **그 다음 변경 요청 전에 CSRF를 다시 받는다**.
