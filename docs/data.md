...
## KT 는 5G/LTE 통합요금제다 (2026-09-20 확인)

KT 요금제 페이지 배너: **"5G/LTE 구분없이 데이터 안심, 연령별 맞춤 혜택은 자동 제공"**
(https://product.kt.com/wDic/index.do?CateCode=6002, 2026-09-20 확인). 통합요금제 탭의 초이스·베이직·음성과
온라인전용 요고가 그 라인업이다.

그래서 백오피스 품질 검사의 `KT · LTE 0건` 은 **수집 결손이 아니라 모델링 결손**이었다. 우리가 KT 71건을
`5G` 로만 적어 둔 탓에 **망을 LTE 로 고른 사용자에게 KT 후보가 0건**이었다 — 쓸 수 있는 선택지를 우리가
숨기고 있었다. CSV 를 `5G/LTE` 로 고치고 DB 는 `LTE_5G` 로 적재한다(D-58).
...
> 무엇을 왜 직접 묻는지(인자 전체 목록·묻는 방식 규칙)는 [사용자 입력](user-inputs.md), 결정은 D-33.
