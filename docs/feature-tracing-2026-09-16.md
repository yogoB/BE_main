...
| Google 캘린더 | ✅ **구현 완료** | — | 회원=담기 열림 · 비회원=로그인 안내 | §3 — 조치 끝(날짜 정교화만 남음) |

**세 기능 모두 "코드가 고장난" 것이 아니었다.** 두 개는 설정이 없고, 하나는 애초에 만든 적이 없었다(§3에서 구현 완료).
...
## 3. Google 캘린더 — **(A) 링크 방식으로 구현 완료 (2026-09-16)**

> 아래 "미구현" 기록은 조사 시점(같은 날 오전)의 상태다. 사용자 결정에 따라 **(A) 링크 방식**으로
> 구현하고 **회원 전용**으로 제한했다(프론트 `47612ad`, 배포 완료).
>
> - 일정별 링크 → `calendar.google.com/calendar/render?action=TEMPLATE&text&dates&details`
>   새 탭에서 추가 화면이 열리고 **저장은 사용자가 한다**. OAuth 권한도 토큰 보관도 없다.
> - 전체 `.ics` 내려받기 → 애플 캘린더·아웃룩용(RFC 5545, 종일 `DTSTART;VALUE=DATE`).
> - **회원 전용**: `GET /api/v1/me` 로 판단해 로그인했을 때만 담기가 열린다. 비회원에게는
>   "로그인하고 캘린더에 담기" 링크를 보여주고, 캘린더 표 자체는 계속 보여준다.
> - 일정 설명에 결과 화면이 넘긴 **실제 추천 요금제명·월 요금·절감액**이 들어간다. 값이 없으면 그 줄을 뺀다.
>
> **남은 한계**: 일정 날짜는 여전히 오늘 기준 고정 오프셋(0·3·8·14·21일)이다.
> 약정 만료일을 쓰려면 `/me/switch-timing` 이 필요하고 그건 "현재 요금제 저장"이 선행된다.
>
> 아래는 조사 당시 기록이다.
...
3. ~~**§3 캘린더**~~ — **(A)로 구현·배포 완료.** 남은 건 날짜 정교화(약정 만료일 연결)뿐이다.

§1·§2는 **코드 변경이 필요 없다.** 자격증명이라 직접 입력해야 한다.
