...
- **런타임 크롤링** — 데이터는 시드 스냅샷으로 고정 (D-05)
  - D-18(+D-43): 카탈로그 값은 검증 후 CSV 에만 반영한다(`docs/catalog-data.md`).
    CSV가 카탈로그 원본이며 **요청 중 외부 가격 조회는 하지 않는다** — 이 금지는 그대로다.
    **수집은 모델이 직접 해도 된다(D-43, 2026-09-17).** 단 1차 출처 URL·원문 인용·확인일이 있어야 하고
    값이 그 인용문에서 직접 읽혀야 한다. 검색 결과 요약·모델의 기억은 여전히 증거가 아니며 운영자 승인도 그대로다.
    우체국 연동은 제거한다. 스마트초이스는 검증 오버레이로 되살림(D-20): 카탈로그 소스 아님, 배치 스냅샷 대조로만 쓴다.
